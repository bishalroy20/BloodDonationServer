...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    if (lastDonationDate) {
      newUser.lastDonationDate = new Date(lastDonationDate);
    }
...
//   await userCollection.updateOne(
//     { uid: req.params.uid },
//     { $set: { name, avatarUrl, bloodGroup, district, upazila , lastDonationDate } }
//   );
//   res.json({ success: true });
// });
...
app.patch("/api/users/:uid", async (req, res) => {
  ...
  const { name, avatarUrl, bloodGroup, district, upazila, lastDonationDate } =
    req.body;
  ...
  await userCollection.updateOne({ uid }, updateDoc);
  res.json({ success: true });
});
...
app.get("/api/stats/total-funding", async (req, res) => {
  ...
  const agg = await fundingCollection
...
app.post("/api/auth/issue-token", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ uid, email, role: user.role }, JWT_SECRET, {
      expiresIn: "7d",
    });
    res.json({ token });
  } catch {
